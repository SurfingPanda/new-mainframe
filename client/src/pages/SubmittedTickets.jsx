...
const ROLES = [
  ...
  { key: 'owner', label: 'I own' },
  { key: 'assignee', label: "I'm assigned" }
];
      ...
      <main className="container-app py-10 space-y-6">
        ...
        <section className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <Link to="/tickets/my-queue" className="btn-secondary !px-3.5 !py-2 text-xs">
          ...
          <div className="flex flex-col gap-3 border-b border-slate-100 p-4 lg:flex-row lg:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search by ID, title, requester, assignee, or description…"
            ...
            <div className="flex flex-wrap gap-2">
              <Select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} label="Your role">
                {ROLES.map((r) => (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50/80">
                <tr className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                  ...
                  <Th className="w-28">Your role</Th>
                  <Th>Title</Th>
                ...
                {loading ? (
                  ...
                  pageRows.map((t) => (
                    <tr key={t.id} className={`hover:bg-slate-50/60 ${ACTIVE_STATUSES.has(t.status) ? '' : 'opacity-70'}`}>
                      ...
                      <td className="px-5 py-3 text-slate-700 truncate">{t.requester || '—'}</td>
                      <td className="px-5 py-3 text-slate-700 truncate">{t.assignee || <span className="text-slate-400">Unassigned</span>}</td>
...
function RolePill({ role }) {
  const map = {
    owner: { label: 'Owner', cls: 'bg-accent-50 text-accent-700 ring-accent-200' },
    assignee: { label: 'Assignee', cls: 'bg-brand-50 text-brand-800 ring-brand-200' },
    both: { label: 'Owner · Assignee', cls: 'bg-amber-50 text-amber-700 ring-amber-200' }
  };
  const r = map[role] || map.owner;
  return (
    <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ring-1 ring-inset ${r.cls}`}>
      {r.label}
    </span>
  );
}
